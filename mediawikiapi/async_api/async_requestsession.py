...
class AsyncRequestSession(BaseRequestSession):
    ...
    def __init__(
        self, pool_size: int = 100, pool_connections_per_host: int = 10
    ) -> None:
    ...
    async def detect_api_version(
        self, api_url: str, config: Config
    ) -> Optional[MediaWikiVersion]:
        """
        ...
        try:
            ...
            async with session.get(
                ...
                if (
                    "query" in data
                    and "general" in data["query"]
                    and "generator" in data["query"]["general"]
                ):
    ...
    async def request(
        ...
        async with self.__request_limiter.global_semaphore, host_semaphore:
            ...
            try:
                ...
                # Respect rate limits
                should_limit, wait_seconds = should_rate_limit(
                    self.__rate_limit_last_call, config
                )
                ...
                query_identifier = str(
                    params.get("titles", params.get("search", "unknown"))
                )
                ...
                while attempt < max_attempts:
                    try:
                        ...
                        if attempt > 0:
                            ...
                            backoff_with_jitter = calculate_backoff_with_jitter(
                                config, attempt - 1
                            )
                        ...
                        async with session.get(
                            ...
                            try:
                                ...
                                if not config.should_retry(attempt, None):
                                    ...
                                    error_context["content_sample"] = (
                                        text_sample[:500] if text_sample else None
                                    )
                        ...
                        if not config.should_retry(attempt, None):
                            ...
                            raise HTTPTimeoutError(
                                query_identifier, timeout=config.timeout
                            )
                        last_exception = e
                ...
                if not config.should_retry(attempt, None):
                    error_context = {"url": api_url, "attempts": attempt + 1}
                    raise NetworkError(
        ...
        if attempt >= max_attempts and last_exception is not None:
            error_context = {"url": api_url, "attempts": attempt}
            raise NetworkError(
