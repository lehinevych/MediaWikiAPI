...
from typing import Any, Dict, Optional, TypeVar, cast, Union
...
class async_memoized_class:
    ...
    def __init__(
        self,
        func: Optional[Callable[..., Coroutine[Any, Any, R]]] = None,
        *,
        ...
        self.cache: Union[
            Dict[str, tuple[R, float]], OrderedDict[str, tuple[R, float]]
        ] = {}
        # We'll convert to OrderedDict later if needed when we know the instance config

        # Handle both @async_memorized and @async_memorized(...) forms
        if func is not None:
    ...
    async def __call__(self, *args: Any, **kwargs: Any) -> R:
        ...
        if self.func is None:
            # Called as @async_memorized(...)
            func = args[0]
            wrapped = async_memoized_class(
                func=func, ttl=self.ttl, max_size=self.max_size
            )
            return cast(R, wrapped)
...
def async_memorized(
    ...
    *,
    ...
    """
    if func is None:
        # Called as @async_memorized(ttl=300, max_size=1000)
        return lambda f: async_memorized(f, ttl=ttl, max_size=max_size)

    # Called as @async_memorized
    memoize = async_memoized_class(func, ttl=ttl, max_size=max_size)
