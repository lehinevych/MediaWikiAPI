...
def validate_title_or_pageid(
    title: Optional[Any] = None, pageid: Optional[Any] = None
) -> None:
    """
...
def validate_limit(limit: Union[int, str], max_value: int = 500) -> None:
    ...
    if isinstance(limit, str):
        ...
        raise ValueError(
            f"Limit must be an integer or 'max'/'max_value', got {type(limit)}"
        )


def validate_category_params(
    title: Optional[str] = None, pageid: Optional[int] = None, cmtype: str = "page"
) -> None:
    """
...
def validate_sentences_and_chars(
    sentences: Optional[int], chars: Optional[int]
) -> None:
    """
