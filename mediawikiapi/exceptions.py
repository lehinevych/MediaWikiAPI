...
class DisambiguationError(MediaWikiAPIException):
    ...
    def __init__(
        self,
        title: str,
        may_refer_to: Optional[List[str]] = None,
        context: Optional[Dict[str, Any]] = None,
    ):
        self.title = title
        self.options = may_refer_to or []
        ctx = context or {}
        ctx.update({"title": title, "options": self.options})
        super().__init__(f'"{title}" is a disambiguation page', ctx)

    def __unicode__(self) -> str:
        msg = f'"{self.title}" may refer to: '
        if self.options:
