...
class TestBasicErrorHandling:
    ...
    def test_handle_api_error_server_error(self):
        ...
        with pytest.raises(ServerError):
            handle_api_error(
                {"error": {"code": "internal_api_error", "info": "Server error"}},
                "test",
            )
    ...
    def test_handle_api_error_generic(self):
        ...
        with pytest.raises(MediaWikiAPIException):
            handle_api_error(
                {"error": {"code": "unknown_error", "info": "Unknown error"}}, "test"
            )
    ...
    def test_handle_page_error(self):
        ...
        with pytest.raises(PageError):
            handle_page_error(
                {"query": {"pages": {"-1": {"missing": ""}}}}, "Nonexistent Page"
            )

        with pytest.raises(PageError):
            handle_page_error(
                {"query": {"pages": {"-1": {"missing": ""}}}}, pageid=12345
            )
    ...
    def test_handle_redirect(self):
        ...
        with pytest.raises(RedirectError):
            handle_redirect(
                {"query": {"redirects": [{"from": "Test", "to": "Test Page"}]}},
                "Test",
                allow_redirect=False,
            )

        # Ensure it doesn't raise when redirects are allowed
        result = handle_redirect(
            {"query": {"redirects": [{"from": "Test", "to": "Test Page"}]}},
            "Test",
            allow_redirect=True,
        )
