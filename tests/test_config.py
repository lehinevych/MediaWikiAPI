...
class TestConfig(unittest.TestCase):
    ...
    def test_default_get_api_url(self) -> None:
        self.assertEqual(Config().get_api_url(), Config.API_URL.format(Language.DEFAULT_LANGUAGE))

    def test_default_get_api_url_with_custom_language(self) -> None:
        fr_lang = "fr"
        uk_lang = "uk"
        config = Config()
        self.assertEqual(config.get_api_url(language=fr_lang), config.API_URL.format(fr_lang))
        self.assertEqual(config.get_api_url(language=Language(uk_lang)), config.API_URL.format(uk_lang))

    def test_default_rate_limit_is_None(self):
        self.assertEqual(Config().rate_limit, None)

    def test_default_donate_url(self):
    ...
    def test_custom_api_url_with_language_support(self) -> None:
        ...
        self.assertEqual(Config(mediawiki_url=api_url).get_api_url(), api_url.format(Language.DEFAULT_LANGUAGE))

    def test_custom_api_url_with_language_support(self) -> None:
        api_url = "https://wiki.archlinux.org"
        self.assertEqual(Config(mediawiki_url=api_url).get_api_url(), api_url)

    def test_custom_rate_limit_as_int(self):
        rate_limit: int = 10
        self.assertEqual(Config(rate_limit=rate_limit).rate_limit, timedelta(milliseconds=10))
    
    def test_custom_rate_limit_as_timedelta(self):
