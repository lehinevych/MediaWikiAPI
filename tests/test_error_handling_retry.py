...
class TestErrorHandlingSync:
    ...
    def test_no_retry_strategy(self, mock_get):
        ...
        api = MediaWikiAPI(
            retry_strategy=Config.RetryStrategy.NONE
        )
...
class TestErrorHandlingAsync:
    ...
    async def test_retry_on_http_error_async(self, mock_get):
        ...
        error_cm = MagicMock()
        error_response = MagicMock()
        error_response.raise_for_status = asyncio.coroutine(lambda: (_ for _ in ()).throw(
            aiohttp.ClientResponseError(None, (), status=500, message="Server Error")
        ))
