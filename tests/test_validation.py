...
class TestValidation(unittest.TestCase):
    ...
    def test_validate_title_or_pageid(self):
        ...
        # Invalid scenarios
        with pytest.raises(ValueError, match="Either a title or a pageid must be specified"):
            validate_title_or_pageid()
            
        with pytest.raises(ValueError, match="Please specify only a title or only a pageid, not both"):
    ...
    def test_validate_geosearch_params(self):
        ...
        with pytest.raises(ValueError, match="Radius must be between 10 and 10000 meters"):
            validate_geosearch_params(
    ...
    def test_validate_limit(self):
        ...
        # Invalid scenarios
        with pytest.raises(ValueError, match="String limit must be 'max' or 'max_value'"):
        ...
        with pytest.raises(ValueError, match="Limit must be an integer or 'max'/'max_value'"):
    ...
    def test_validate_category_params(self):
        ...
        # Invalid scenarios
        with pytest.raises(ValueError, match="Please specify only a category title or only a pageid, not both"):
            validate_category_params(title="Science", pageid=12345)
            
        with pytest.raises(ValueError, match="Either a category title or a pageid must be specified"):
            validate_category_params()
            
        with pytest.raises(ValueError, match="Category member type must be one of: 'page', 'subcat', or 'file'"):
    ...
    def test_validate_sentences_and_chars(self):
        ...
        # Invalid scenarios
        with pytest.raises(ValueError, match="Number of sentences must be non-negative"):
            validate_sentences_and_chars(sentences=-1, chars=None)
            
        with pytest.raises(ValueError, match="Number of characters must be non-negative"):
            validate_sentences_and_chars(sentences=None, chars=-1)
            
        with pytest.raises(ValueError, match="Specify only one of sentences or chars, not both"):
